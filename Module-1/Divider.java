...
public class Divider{
    ...
    public static double divide(int i1, int i2) throws ArithmeticException{
        if (i2 == 0) {
            ...
            return (double) i1/i2;  // (double) has to be precised to the java calculator
        }
    }
    /*
    ...
    public static String betterCanDivide(int i1, int i2){
        try {
            ...
            return Double.toString(result);  // toString because the double result has to be returned as a string
        } catch (ArithmeticException e){
            return e.getMessage(); // to get the message contained by the ArithmeticException's throw
        }
    }
